...
describe('World level system', () => {
  it('lists and successfully loads static cities into indexed runtime data', async () => { expect(levelRegistry.listLevels().map(({ id }) => id)).toEqual(['test-city', 'mini-city']); const world = await levelRegistry.loadLevel('test-city'); const mini = await levelRegistry.loadLevel('mini-city'); expect(world.definition.roads).toHaveLength(28); expect(world.buildingsById.size).toBe(48); expect(mini.definition.metadata.name).toBe('Mini Junction'); });
