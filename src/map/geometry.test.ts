...
describe('geographic utilities', () => {
  ...
  it('creates complete GeoJSON sources in MapLibre longitude/latitude order', async () => { const world = await levelRegistry.loadLevel('test-city'); const sources = createBaseSources(world); expect(Object.keys(sources).sort()).toEqual(['acquisition-costs', 'bounds', 'buildings', 'context', 'pois', 'population', 'roads', 'water', 'workplaces']); expect(sources.roads.features[0].geometry).toMatchObject({ type: 'LineString', coordinates: [[-87.64, 41.868], [-87.64, 41.882]] }); expect(sources.buildings.features).toHaveLength(48); expect(sources['acquisition-costs'].features).toHaveLength(48); expect(sources['acquisition-costs'].features[0].geometry.type).toBe('Point'); expect(sources.water.features).toHaveLength(1); expect(sources.population.features).toHaveLength(12); expect(sources.workplaces.features).toHaveLength(9); expect(sources.pois.features).toHaveLength(8); expect(sources.context.features).toHaveLength(0); });
  it('provides persistent Dallas district, park, and airport context', async () => { const world = await levelRegistry.loadLevel('dallas'); const context = createBaseSources(world).context.features; const names = context.map((feature) => feature.properties?.name); expect(names).toContain('DOWNTOWN'); expect(names).toContain('DFW INTERNATIONAL AIRPORT'); expect(names).toContain('WHITE ROCK LAKE'); expect(context.some((feature) => feature.properties?.kind === 'park')).toBe(true); });
});
