...
const workplaces = [...clusters(config.sources.workplaces, 'workplace', 'jobs', config.workplaceRules.jobsProperty).slice(0, config.workplaceRules.maxClusters), ...config.manualOverrides.workplaces.map(({ id, name, latitude, longitude, jobs }) => ({ id, coordinate: { latitude, longitude }, jobs, displayName: name }))];
...
writeFileSync(resolve(dirname(output), `${config.cityId}.attribution.json`), `${JSON.stringify({ cityId: config.cityId, pipeline: 'tools/real-city/src/cli.mjs', processedAt: new Date().toISOString().slice(0, 10), sources: config.sources, sourceMetadata: config.sourceMetadata ?? {}, warnings: ['Workplace clusters use a public employer directory in this generated build; replace with LODES WAC aggregation when a suitable extract is available.'] }, null, 2)}\n`);
